...
class QmDataContainer:
    ...
    def __init__(
        ...
        assert self.U0 is not None
        # assert self.F is not None

        assert len(self.U0) > 0
        # assert len(self.F) > 0

        self.U0 = self.U0[:, None]  # shape=(nMolecules,1)
    ...
    def get_dtypes(self):
        ...
        dtypes_target = {}
        dtypes_target["U0"] = torch.float32
...
if __name__ == "__main__":
    qm9_dataset = "qm9_eV.npz"
    triplets_only = False
    direct_forces = False
    cutoff = 5.0
    int_cutoff = 10.0
    data_container = QmDataContainer(qm9_dataset, cutoff=cutoff, int_cutoff=int_cutoff, triplets_only=triplets_only)
